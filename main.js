...
// DOM Elements: Get references to the location, NORAD input fields, and the search button
   document.getElementById("search").addEventListener("click", function () {
    ...
    if (!locationInput || !noradInput) {
        alert("Please enter both location and a NORAD satellite code.");
        return;
    }

    const encodedLocation = encodeURI(locationInput);
 const geoUrl = `https://geocode.maps.co/search?q=${locationInput},&api_key=${key}`
    ...
    fetch(geoUrl)
        ...
        .then(function (geoData) {
            ...
            document.getElementById("coordinates").innerHTML =
                `<strong>Latitude:</strong> ${lat} <br> <strong>Longitude:</strong> ${lon}`;

            const satUrl = "https://satellites.fly.dev/passes/" + noradInput + "?lat=" + lat + "&lon=" + lon + "&limit=1&days=15&visible_only=true";
        ...
        .then(function (satData) {
            ...
            document.getElementById("satellite-info").innerHTML = `
                ...
                <strong>Culmination:</strong> ${culminationTime.toLocaleString()}<br>
                <strong>Set:</strong> ${setTime.toLocaleString()}
            `;
        })
